...
function getBucket(country) {
  ...
  if (['DE','FR','NL','BE','CH','PL','ES','IT','GR','AT','IE','PT','CZ','RO','HU','NO','SE','FI','DK','IS','GB','EE','LT','LV','HR','RS','BG','UA','BA','ME','MK','AL','MD','BY','SK','SI'].includes(country)) return 'EU';
  if (['JP','KR','SG','AU','TW','MY','ID','TH','VN','PH','IN','HK','NZ','CN','BD','LK','MM','KH','LA','MN','FJ','PG'].includes(country)) return 'Tier5_APAC';
...
function normalize(f) {
  return {
    ...
    gpu_evidence_type: null, compute_vendor: null, rack_density_kw: null, notes: f.notes || ''
  };
}

const w3files = ['new-canada-w2', 'new-us-emerging-w3', 'new-global-emerging-w3'];
let added = 0, skipped = 0;

for (const file of w3files) {
...
console.log(`\nWAVE 3: Added ${added} | Skipped ${skipped}`);
