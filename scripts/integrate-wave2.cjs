...
function normalize(f) {
  return {
    ...
    state_province_region: f.state_province_region || f.region || '',
    city: f.city || '',
    ...
    mw_verified: f.mw_verified || (f.facility_mw != null),
    ...
    confidence_score: f.confidence_score || 60,
    verification_tier: f.verification_tier || 'moderate',
    last_verified_date: '2026-04-04',
    source_1_url: f.source_1_url || f.source_url || null,
...
const w2files = ['new-canada-w2','new-us-mid-w2','new-nordics-uk-w2','new-eu-continental-w2','new-apac-w2','new-mea-latam-w2'];
let added = 0, skipped = 0;

for (const file of w2files) {
  ...
  for (const raw of data) {
    ...
    if (c === 'CA') ca.push(norm);
    else if (c === 'US') us.push(norm);
    else eu.push(norm);
...
console.log(`\nWAVE 2 INTEGRATION: Added ${added} | Skipped ${skipped}`);
